define(["require", "exports", "esri/core/tsSupport/declareExtendsHelper", "esri/core/tsSupport/decorateHelper", "tslib", "esri/core/accessorSupport/decorators", "esri/widgets/Widget", "esri/tasks/IdentifyTask", "esri/tasks/support/IdentifyParameters", "esri/geometry/Point", "esri/geometry/Polyline", "esri/geometry/geometryEngine", "esri/geometry/SpatialReference", "esri/Graphic", "esri/symbols/PictureMarkerSymbol", "esri/tasks/support/Query", "esri/layers/FeatureLayer", "esri/renderers/SimpleRenderer", "esri/symbols/PolygonSymbol3D", "dojo/on", "dojo/dom", "dojo/Deferred", "dojo/query", "dojo/_base/array", "dojo/dom-construct", "dojo/dom-class", "dojo/dom-attr", "dojo/promise/all", "./viewModels/ObstructionViewModel", "esri/widgets/support/widget"], function (require, exports, __extends, __decorate, tslib_1, decorators_1, Widget, IdentifyTask, IdentifyParameters, Point, Polyline, geometryEngine, SpatialReference, Graphic, PictureMarkerSymbol, Query, FeatureLayer, SimpleRenderer, PolygonSymbol3D, on, dom, Deferred, query, Array, domConstruct, domClass, domAttr, all, ObstructionViewModel_1, widget_1) {
    ...
    var ObstructionPane = (function (_super) {
        ...
        ObstructionPane.prototype.performQuery = function (_x, _y, _z, _height) {
            ...
            this.doIdentify(_x, _y).then(function (response) {
                if (response.results) {
                    _this.addToMap(response.results, event);
                }
                else {
        ...
        ObstructionPane.prototype.querySurfaces = function (vertical_line) {
            ...
            var viz = Array.map(crit_3d.layers.items, function (lyr) {
            ...
            var viz2 = Array.map(part77.layers.items, function (lyr) {
                var deferred = new Deferred();
                lyr.queryFeatures(query).then(function (e) {
                    if (e.features.length) {
                        _this.filterSurfaces3D(e, vertical_line).then(function (oids) {
        ...
        ObstructionPane.prototype.addToMap = function (_results, _event) {
            var _this = this;
            var map = this.scene;
            var view = this.view;
            var bldgResults = { displayFieldName: null, features: [] };
            var parcelResults = { displayFieldName: null, features: [] };
            var groundElev = 0;
            for (var i = 0, il = _results.length; i < il; i++) {
                var idResult = _results[i];
            ...
            var obst_height = parseInt(dom.byId("obsHeight").value, 10);
            if (!groundElev) {
                groundElev = parseInt(dom.byId("groundLevel").value, 10);
            }
            var x_value = idParams.geometry.x;
            var y_value = idParams.geometry.y;
            var tab_content1 = this.layerTabContent(bldgResults, "bldgResults", groundElev, obst_height, x_value, y_value);
            var tab_content2 = this.layerTabContent(parcelResults, "parcelResults", groundElev, obst_height, x_value, y_value);
        ...
        ObstructionPane.prototype.layerTabContent = function (layerResults, layerName, base_height, peak_height, x, y) {
            ...
            switch (layerName) {
                case "bldgResults":
                    ...
                    var sum = domConstruct.toDom("<i>3D surfaces affected: " + layerResults.features.length + "</i>");
                    ...
                    var _row = domConstruct.create("tr", null, bldg_table);
                ...
                case "parcelResults":
                    ...
                    var header_row = domConstruct.create("tr", null, table);
                    ...
                    for (i = 0; i < layerResults.features.length; i++) {
                        if (layerResults.features[i].attributes.RWY) {
                            var _row_ = domConstruct.create("tr", null, table);
                            var _td = domConstruct.create("td", { "innerHTML": layerResults.features[i].attributes.RWY + " " + layerResults.features[i].attributes.Layer }, _row_);
                            ...
                            var td = domConstruct.create("td", { "innerHTML": layerResults.features[i].attributes.NAME + " TSA" }, row);
